...
def load_config(model_name, seed):
    """
    Load model configuration for a given model name.

    Resolution order:
      1. ``config.config_{model_name}``
      2. ``config_trial.config_{model_name}``

    This allows using names like ``EEGNet_plain`` / ``ShallowConvNet_adv``
    directly on the command line (e.g. ``-m ShallowConvNet_adv``) without
    having to duplicate shim files under ``config/``.
    """
    try:
        cfg_mod = importlib.import_module(f"config.config_{model_name}")
    except ModuleNotFoundError:
        cfg_mod = importlib.import_module(f"config_trial.config_{model_name}")
